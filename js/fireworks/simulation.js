...
const shapeDefinitions = {
	...
	"__龙舟__": function (ctx, size) {
		const w = size * 1.4, h = size;
		// 船身
		ctx.beginPath();
		ctx.moveTo(w * 0.05, h * 0.55);
		ctx.quadraticCurveTo(w * 0.5, h * 0.85, w * 0.95, h * 0.55);
		ctx.lineTo(w * 0.85, h * 0.45);
		ctx.quadraticCurveTo(w * 0.5, h * 0.7, w * 0.15, h * 0.45);
		ctx.closePath();
		ctx.fill();
		// 龙头
		ctx.beginPath();
	...
	"__艾叶__": function (ctx, size) {
		const w = size * 0.7, h = size;
		// 叶片
		ctx.beginPath();
		ctx.moveTo(w * 0.5, 0);
		ctx.quadraticCurveTo(w, h * 0.3, w * 0.7, h * 0.6);
		ctx.quadraticCurveTo(w * 0.5, h * 0.8, w * 0.3, h * 0.6);
		ctx.quadraticCurveTo(0, h * 0.3, w * 0.5, 0);
		ctx.fill();
		// 叶脉
	...
	"__香囊__": function (ctx, size) {
		const w = size * 0.8, h = size;
		// 囊体
		ctx.beginPath();
		ctx.moveTo(w * 0.2, h * 0.25);
		ctx.quadraticCurveTo(w * 0.1, h * 0.6, w * 0.5, h * 0.9);
		ctx.quadraticCurveTo(w * 0.9, h * 0.6, w * 0.8, h * 0.25);
		ctx.closePath();
		ctx.fill();
		// 收口
...
function getShapeDots(shapeKey) {
	const size = MyMath.randomInt(60, 90);
	const cacheKey = `${shapeKey}:${size}`;
	if (!wordDotCache.has(cacheKey)) {
		const canvas = document.createElement("canvas");
		const ctx = canvas.getContext("2d");
		const padding = 10;
		const drawFn = shapeDefinitions[shapeKey];
		const shapeW = shapeKey === "__龙舟__" ? size * 1.4 + padding * 2 : size + padding * 2;
		const shapeH = size + padding * 2;
		canvas.width = shapeW;
		canvas.height = shapeH;
		ctx.fillStyle = "#fff";
		ctx.save();
		...
		const dots = [];
		const density = appConfig.wordPointDensity;
		...
		wordDotCache.set(cacheKey, { width: canvas.width, height: canvas.height, points: dots });
	}
...
function getWordDots(word) {
	...
	if (shapeDefinitions[word]) {
		return getShapeDots(word);
	}
